...
/*   Updated: 2024/12/10 15:32:57 by rcarbonn         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	flood_fill(char **map_copy, int row, int col, t_map map)
{
	if (row < 0 || row >= map.height || col < 0
		|| col >= (int)ft_strlen(map_copy[row]))
		return (FALSE);
	if (map_copy[row][col] == ' ')
		return (FALSE);
	if (map_copy[row][col] == '1' || map_copy[row][col] == 'F')
		return (TRUE);
	map_copy[row][col] = 'F';
	if (!flood_fill(map_copy, row + 1, col, map) || !flood_fill(map_copy, row
			- 1, col, map) || !flood_fill(map_copy, row, col + 1, map)
